...
interface DnsQuestion {
    ...
    private class ByteBacked(
        ...
        override val qType: DnsResourceRecordType by lazy {
            DnsResourceRecordType[_rawData.getUShortAt(_offset + qName.length)]
        }
        override val qClass: DnsClass by lazy {
            DnsClass[_rawData.getUShortAt(_offset + qName.length + Short.SIZE_BYTES)]
        }
    ...
    private class FieldBacked(
        ...
        override fun toByteArray(buf: ByteArray, offset: Int): ByteArray {
            ...
            cursor += qName.length
            buf.setUShortAt(cursor, qType.value)
            cursor += Short.SIZE_BYTES
            buf.setUShortAt(cursor, qClass.value)
            return buf
        }
    }
}
